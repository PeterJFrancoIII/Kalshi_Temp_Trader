...
def test_weather_status_serialization():
    ...
    with patch("ingestion.weather_status_writer.STATUS_FILE", status_file):
        status = {
...
def test_stale_data_flag():
    ...
    # Test not stale
    with patch("ingestion.weather_status_writer.fetch_wrh_timeseries", return_value={"mock": "data"}), \
         patch("ingestion.weather_status_writer.parse_wrh_timeseries", return_value=[obs_now]), \
         patch("ingestion.weather_status_writer.fetch_nws_forecast", return_value=None), \
    ...
    # Test stale
    with patch("ingestion.weather_status_writer.fetch_wrh_timeseries", return_value={"mock": "data"}), \
         patch("ingestion.weather_status_writer.parse_wrh_timeseries", return_value=[obs_old]), \
         patch("ingestion.weather_status_writer.fetch_nws_forecast", return_value=None), \
         patch("os.path.exists", return_value=False):
...
def test_observed_max_so_far():
    ...
    with patch("ingestion.weather_status_writer.fetch_wrh_timeseries", return_value={"mock": "data"}), \
         patch("ingestion.weather_status_writer.parse_wrh_timeseries", return_value=[o1, o2]), \
         patch("ingestion.weather_status_writer.fetch_nws_forecast", return_value=None), \
         patch("os.path.exists", return_value=False):
...
def test_history_record_count():
    ...
    with patch.object(Path, "exists", return_value=True), \
         ...
         patch("ingestion.weather_status_writer.fetch_wrh_timeseries", return_value=None), \
         patch("ingestion.weather_status_writer.fetch_obhistory", return_value=None), \
         patch("ingestion.weather_status_writer.fetch_nws_forecast", return_value=None):
