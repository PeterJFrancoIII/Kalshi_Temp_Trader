...
The NWS KMIA station remains the settlement/verification target. This module
normalizes TWC point forecast data into a local snapshot for comparison beside
NWS live data in the console.
"""
...
DEFAULT_ENDPOINTS = {
    ...
    "hourly_forecast": os.getenv("TWC_HOURLY_FORECAST_PATH", "/v3/wx/forecast/hourly/48hour"),
}
...
def derive_features(daily: List[Dict[str, Any]], hourly: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for row in hourly:
        ...
        if isinstance(deg, (int, float)) and 80 <= deg <= 150:
            ...
            break
    return {
        "forecast_high_f": daily_max if daily_max is not None else (max(hourly_temps) if hourly_temps else None),
